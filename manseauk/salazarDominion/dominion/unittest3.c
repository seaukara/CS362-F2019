...
int checkDiscardedHand(struct gameState *pre, int currentPlayer) {
    ...
    if (pre->discardCount[currentPlayer]+pre->handCount[currentPlayer] == post.discardCount[currentPlayer]) {
        return 1;
    } else {
        return 0;
    }
...
int main(int argc, char** argv) {
    ...
    if (checkDiscardedHand(&game2, currentPlayer) == 1)
    {
